...
export default function AjrakhCollection() {
  const { currency } = useCurrency();
  const [products, setProducts] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/api/products?category=ajrakh-block-print-stories')
      .then(res => res.json())
      .then(data => {
    ...
    <main style={{ backgroundColor: '#0a0a0a', color: '#fcfaf8', minHeight: '100vh' }}>
      ...
      <section style={{ position: 'relative', width: '100%', height: '90vh', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 0 }}>
          <img src="/images/ajrakh_hero.png" alt="Folded Ajrakh textiles and hand-carved blocks" style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.6 }} />
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '800px', padding: '2rem' }}>
          <h1 style={{ fontSize: '4.5rem', fontFamily: 'var(--font-serif)', marginBottom: '1.5rem', lineHeight: 1.1, color: '#d4af37' }}>
            Ajrakh & Block Print Stories
          </h1>
          <p style={{ fontSize: '1.2rem', lineHeight: 1.6, opacity: 0.9, marginBottom: '3rem', maxWidth: '600px', margin: '0 auto 3rem auto' }}>
            Earth, indigo, hand-carved blocks, and slow textile rhythm — Indian print traditions for the modern wardrobe and home.
          </p>
      ...
      <section style={{ padding: '3rem 0', borderBottom: '1px solid rgba(212,175,55,0.2)', borderTop: '1px solid rgba(212,175,55,0.2)' }}>
        <div className="container" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '2rem', justifyItems: 'center' }}>
          {[
            { icon: '🌿', text: 'Craft-Origin Verified' },
      ...
      <section id="craft-story" style={{ padding: '8rem 2rem', backgroundColor: '#0a0a0a' }}>
        <div className="container" style={{ maxWidth: '1000px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }}>
            <div>
              <h2 style={{ fontSize: '3rem', fontFamily: 'var(--font-serif)', color: '#d4af37', marginBottom: '1.5rem' }}>The Rhythm of the Block</h2>
              <p style={{ fontSize: '1.2rem', lineHeight: 1.8, opacity: 0.9 }}>
                From hand-carved blocks to layered prints, every piece carries the patience of artisans and the poetry of repetition. IDFIS curates Ajrakh and block-printed textiles across scarves, apparel, table linen, and home accents for modern craft living.
              </p>
            </div>
            <div>
              <img src="/images/ajrakh_story.png" alt="Artisan hands block-printing fabric" style={{ width: '100%', height: 'auto', border: '1px solid rgba(212,175,55,0.2)' }} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="shop-collection" style={{ padding: '6rem 2rem', backgroundColor: '#111', borderTop: '1px solid rgba(212,175,55,0.2)' }}>
        <div className="container">
          <div style={{ textAlign: 'center', marginBottom: '5rem' }}>
            <h2 style={{ fontSize: '3rem', fontFamily: 'var(--font-serif)', color: '#d4af37', marginBottom: '1rem' }}>Featured Prints</h2>
          </div>
      ...
      <section style={{ padding: '8rem 2rem', backgroundColor: '#0a0a0a' }}>
        <div className="container" style={{ maxWidth: '1200px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
            {[
              { icon: '🧱', title: 'Hand-Carved Blocks', desc: 'Each motif begins with a carved block and patient repetition.' },
              { icon: '🪴', title: 'Earthy Dye Character', desc: 'Indigo, madder, and natural tones create depth and softness.' },
              { icon: '🎁', title: 'Wearable & Giftable', desc: 'Versatile textiles for fashion, home, travel, and thoughtful gifting.' }
            ].map((feature, idx) => (
      ...
      <section style={{ padding: '6rem 2rem', backgroundColor: '#111', borderTop: '1px solid rgba(212,175,55,0.2)' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '2rem' }}>
            {[
              { title: 'Thoughtful Gifting', desc: 'Artful textiles, beautifully wrapped for people who value craft and story.', img: '/images/ajrakh_curated_1.png', cta: 'Shop Scarves & Stoles' },
              { title: 'Home with Handprints', desc: 'Bring artisanal warmth to your home with timeless block-printed accents.', img: '/images/ajrakh_curated_2.png', cta: 'Shop Home Edit' },
              { title: 'Premium Packaging', desc: 'Secure, elegant packaging designed for a refined unboxing experience.', img: '/images/ajrakh_curated_3.png', cta: 'Explore Packaging' }
            ].map((edit, idx) => (
      ...
      <section style={{ padding: '8rem 2rem', backgroundColor: '#0a0a0a', borderTop: '1px solid rgba(212,175,55,0.2)' }}>
        <div className="container" style={{ maxWidth: '1000px', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '3rem', fontFamily: 'var(--font-serif)', color: '#d4af37', marginBottom: '3rem' }}>Care for Handprinted Textiles</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', textAlign: 'left', background: '#111', padding: '4rem', border: '1px solid rgba(212,175,55,0.2)' }}>
            <div>
              ...
              <p style={{ fontSize: '1rem', lineHeight: 1.6, opacity: 0.8 }}>
                Wash gently in cold water where suitable, or dry clean if the product label recommends it. Dry in shade, avoid harsh detergents, and keep away from prolonged direct sunlight. Minor print variations, soft irregularities, and tonal differences are part of the handmade character.
              </p>
            </div>
            <div>
              <h3 style={{ fontSize: '1.5rem', fontFamily: 'var(--font-serif)', color: '#d4af37', marginBottom: '1rem' }}>Packaging Reassurance</h3>
              <p style={{ fontSize: '1rem', lineHeight: 1.6, opacity: 0.8 }}>
                Each block-printed textile is folded carefully, wrapped to protect the surface, and packed according to product type, weight, and destination.
              </p>
            </div>
          </div>
        </div>
